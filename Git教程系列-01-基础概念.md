...
等学到远程协作后，可以把它扩展成四个阶段：

```mermaid
flowchart LR
    A[工作目录<br/>正在编辑] -->|git add| B[暂存区<br/>下一次提交的候选内容]
    B -->|git commit| C[本地仓库<br/>已经保存的提交]
    C -->|git push| D[远程仓库<br/>别人可以获取的提交]
    D -->|git fetch / git pull| C
```

这张图有一个很重要的边界：`git commit` 只到本地仓库，`git push` 才会把提交送到远程仓库。
