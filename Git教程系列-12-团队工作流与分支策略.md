...
持续集成（CI）不是工作流最后的一枚印章，而是倒逼团队更早发现问题的反馈机制。分支设计要服务这个目标：让改动尽早和别人正在做的事相遇，并且用自动化检查判断能不能继续往前走。

可以把常见分支按“反馈位置”理解：

| 分支或阶段 | 主要问题 | CI 应该做什么 |
|---|---|---|
| 个人功能分支 | 我这次改动自己能不能跑通？ | 跑单元测试、lint、基础构建 |
| PR/MR 分支 | 这次改动能不能和目标分支一起工作？ | 基于目标分支重新构建、跑核心测试 |
| QA/集成分支 | 多个即将发布的功能放在一起是否冲突？ | 跑更完整的回归、部署到测试环境 |
| `main` / `master` | 是否可以发布或部署？ | 只接受已经通过检查的提交，失败时快速阻断 |

书里提到的 Branch Per Feature 和 QA 分支可以这样吸收：功能分支要小，合并前通过自动化检查；如果团队确实需要把多个功能先放在一起验收，可以设置短生命周期 QA/集成分支。但不要把 QA 分支变成第二条长期主线。它应该随着一个迭代或一个发布窗口结束而清理，否则会变成新的冲突来源。

一个实用判断是：如果你的分支策略让 CI 只能在最后一天才发现大面积冲突，这套策略就太晚了；如果每个小改动都能尽早被测试、审查和集成，哪怕流程名字很朴素，也比复杂但反馈迟钝的流程更健康。
