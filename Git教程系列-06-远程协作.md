...
更底层地说，`git clone` 会复制远程仓库中你需要的 Git 对象和引用，然后检出一个工作目录给你编辑。它不是只下载“当前最新版文件”，所以克隆下来的仓库通常可以离线查看旧提交、创建分支、比较历史。也正因为每个正常 clone 都带着完整历史，它可以作为灾难恢复的一个来源；但团队仍然应该有正式备份、权限和远程仓库治理，不要把“某个人电脑上有 clone”当成唯一备份策略。
...
可以把 push 理解成两步：先把远程还没有的提交对象传过去，再请求远程把某个分支名移动到新的提交上。远程如果发现这样会让它丢掉已有历史，就会拒绝普通 push，这就是很多 `non-fast-forward` 错误的根源。
...
换句话说，`fetch` 是从另一个仓库拿到“我还没有的对象和引用位置”。它会更新 `origin/main` 这类远程跟踪分支，但不会替你决定要不要把这些提交合进 `main`。
...
| `push` 的反义词是 `pull` | 不准确。`push` 对应的是把对象和引用送出去；真正相反的获取动作是 `fetch`，`pull` 是 fetch 之后再 merge 或 rebase |
