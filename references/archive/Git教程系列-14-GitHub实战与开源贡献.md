...
4. 跑通开源贡献的标准流程
5. 知道 issue 和 bug report 应该提供哪些信息
...
好的 bug report 不是“这里坏了”，而是让维护者能复现、判断影响范围，并决定下一步优先级。

| 信息 | 说明 | 示例 |
|---|---|---|
| 标题 | 短而具体，说明位置和现象 | “Windows 下 `git status` 输出中文路径乱码” |
| 环境 | 操作系统、Git 版本、平台或项目版本 | Windows 11、Git 2.45、Git Bash |
| 复现步骤 | 一步一步写，不跳过前提 | clone 项目 → 执行某命令 → 打开某文件 |
| 预期结果 | 你认为应该发生什么 | 路径正常显示中文 |
| 实际结果 | 实际看到了什么 | 终端出现乱码或问号 |
| 证据 | 截图、日志、最小仓库或命令输出 | 贴出相关错误，不贴无关长日志 |
| 影响范围 | 严重程度和是否阻塞 | 只影响显示，还是导致无法提交 |

如果你能先搜索已有 issue，确认不是重复问题，会节省维护者很多时间。严重程度可以由报告者建议，但优先级通常由维护者根据项目计划、里程碑和可用人力决定。
...
1. 先读 README 和贡献指南。
2. 报 bug 前先搜索已有 issue，能复现时写清楚环境、步骤、预期和实际结果。
3. 小改动可以直接 PR，大改动先开 issue 讨论。
4. PR 描述写清楚背景和验证。
5. 不要催维护者立刻合并。
6. review 要求修改时，继续在同一分支提交并 push。
7. 不要把 unrelated changes 混进 PR。
8. 保持自己的 fork 主分支接近 upstream 主分支，功能改动放在短生命周期分支里。
