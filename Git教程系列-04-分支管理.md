...
| 层次 | 建议掌握 |
|---|---|
| 先读 | 分支为什么存在、`HEAD` 指向哪里、如何创建和切换分支 |
| 再读 | 分支图、删除/重命名分支、远程分支预告和常见误解 |

读完第一层，你已经能安全地用功能分支开发；第二层帮助你以后看懂复杂历史。
...
从 Git 内部看，本地分支通常就是 `.git/refs/heads/` 下面的一条引用，里面记录着某个提交哈希。创建分支很便宜，是因为 Git 主要是在新增一个“名字指向提交”的记录；真正保存项目内容的是提交背后的对象库，而不是为每个分支复制一整份文件夹。
...
在使用分支时,经常需要引用"某个提交"。Git 提供了多种方式。

### 常见引用方式

| 引用方式 | 含义 | 示例 |
|---|---|---|
| **完整哈希** | 提交的完整 SHA-1 | c82a22c39cbc32576f64f5c6b3f24b99ea8149c7 |
| **短哈希** | 前几位字符(足以唯一标识) | c82a22c |
| **分支名** | 分支指向的提交 | main, feature |
| **HEAD** | 当前所在位置 | HEAD |
| **父提交** | 上一个/上 N 个提交 | HEAD^, HEAD~3 |
| **标签** | 给提交起的别名 | v1.0, v2.5 |

### 父提交引用

`^` 和 `~` 都可以引用父提交,但有细微区别:

**HEAD^** (或 HEAD^1): 第一个父提交
...
```bash
git show HEAD^^  # 查看上上个提交
```

**HEAD~N**: 往上数 N 个提交

```bash
git show HEAD~3  # 查看往上数第3个提交
```
...
- `E^1` 或 `E^`: 第一个父提交 (main 分支的 C)
- `E^2`: 第二个父提交 (feature 分支的 D)
...
# 基于上一个提交创建新分支
git switch -c fix-bug HEAD^

# 回退到上一个提交 (保留工作区改动)
git reset HEAD^
```

这些引用方式在后面的章节 (撤销、查询、rebase) 会频繁用到。现在先有个印象,遇到时再回来查。
