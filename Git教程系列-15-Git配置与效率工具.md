...
1. 理解 Git 配置的层级
2. 配好用户名、邮箱、默认分支、换行符、编辑器和提交模板
3. 认识 `push.default` 和命令纠错配置的安全边界
4. 使用 alias 简化高频命令
5. 理解 CLI、GUI、VS Code 和 hooks 的定位与边界
...
如果团队经常要求提交说明包含背景、变更范围或工单号，可以配置提交模板，而不是每次从零想格式。

先创建一个模板文件，例如 `~/.gitmessage`：
...
Git 会打开编辑器，并把模板内容放进去。模板适合提醒你写清楚提交，不适合把提交说明变成填表负担。如果只是个人学习项目，一句清楚的 `git commit -m "修复登录按钮样式"` 已经够用。
...
| hook | 位置 | 触发时机 | 能否拦截 | 常见用途 |
|---|---|---|---|---|
| `pre-commit` | 客户端 | 提交前 | 能 | 格式化、lint、禁止提交密钥 |
| `commit-msg` | 客户端 | 提交信息生成后 | 能 | 检查提交信息格式 |
| `post-commit` | 客户端 | 提交后 | 不能 | 本地通知、生成辅助记录 |
| `pre-push` | 客户端 | 推送前 | 能 | 跑测试或安全检查 |
| `pre-receive` | 服务端 | 接收 push 前 | 能 | 检查权限、保护分支、拒绝不合规提交 |
| `update` | 服务端 | 每个引用更新时 | 能 | 对单个分支或标签做策略检查 |
| `post-receive` | 服务端 | 接收 push 后 | 不能 | 通知 CI、发消息、触发部署 |
